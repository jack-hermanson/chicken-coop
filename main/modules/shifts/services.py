...
from utils.date_functions import day_of_week_str, get_next_date_with_same_day_of_week, time_of_day_str
from utils.date_time_enums import DayOfWeekEnum, TimeOfDayEnum

from .forms import AssignRecurringShiftForm, AssignSpecificShiftForm, ShiftInstanceCompletedTimestampForm
...
from .view_models import AssignShiftViewModel, ShiftInstanceViewModel


def get_future_shift_instances(shift_id):
    return (
        ShiftInstance.query.filter(and_(ShiftInstance.due_date >= date.today(), ShiftInstance.shift_id == shift_id))
...
def get_previous_shifts(page: int):
    ...
    """
    shift_instances = (
        ShiftInstance.query.join(ShiftInstance.shift)
        .filter(
...
def generate_shift_instance_view_model(shift_instance: ShiftInstance, default_name: str) -> ShiftInstanceViewModel:
    due_date_is_within_editable_range = (
        # today
        shift_instance.due_date.date() == datetime.today().date()
        or (
            # or yesterday
    ...
    view_model = ShiftInstanceViewModel(
        ...
        form,
        sunrise=(
            shift_instance.sunrise_utc.replace(tzinfo=from_zone).astimezone(to_zone).strftime("%l:%M %p")
            if shift_instance.sunrise_utc
            else None
        ),
        sunset=(
            shift_instance.sunset_utc.replace(tzinfo=from_zone).astimezone(to_zone).strftime("%l:%M %p")
            if shift_instance.sunset_utc
            else None
        ),
        target_time=(
            (shift_instance.sunrise_utc + timedelta(minutes=30))
            .replace(tzinfo=from_zone)
            .astimezone(to_zone)
            .strftime("%l:%M %p")
            if shift_instance.sunrise_utc is not None
            else None
        ),
        # target_time=(
...
def get_shifts_that_need_signups():
    ...
    shifts_that_need_signups = (
        ...
        .order_by(Shift.day_of_week, Shift.time_of_day)
        .all()
    )
...
def generate_alert_for_shifts_that_need_signups():
    ...
    if len(shifts_that_need_signups):
        logger.info(
            f"There {'is' if len(shifts_that_need_signups) else 'are'} {len(shifts_that_need_signups)} "
...
def assign_specific_shift(assign_specific_shift_form: AssignSpecificShiftForm) -> SpecificShiftInstanceAssignment:
    ...
    existing_shift_instance: ShiftInstance = (
        ShiftInstance.query.join(ShiftInstance.shift)
        .filter(
            and_(
    ...
    if existing_specific_shift_instance_assignment:
        logger.warn(
            f"Updating existing specific shift instance assignment with ID "
            f"{existing_specific_shift_instance_assignment.specific_shift_instance_assignment_id}"
        )
        logger.debug(
            f"{existing_specific_shift_instance_assignment.instance_date.strftime('%F')} "
    ...
    logger.info(
        "Adding a new specific_shift_instance_assignment %s %s",
        assign_specific_shift_form.date.data,
...
def get_specific_shift_instance_assignments() -> list[SpecificShiftInstanceAssignment]:
    return SpecificShiftInstanceAssignment.query.order_by(SpecificShiftInstanceAssignment.instance_date.desc())


def get_paginated_specific_shift_instance_assignments(page: int):
    specific_shift_instance_assignments = SpecificShiftInstanceAssignment.query.order_by(
...
def get_average_eggs_for_all_shifts():
    return (
        db.session.query(func.avg(ShiftInstance.eggs_taken_home))
        .filter(ShiftInstance.eggs_taken_home.is_not(None))
        .scalar()
    )


def get_average_eggs_for_day_and_time(day_of_week: DayOfWeekEnum, time_of_day: TimeOfDayEnum, weeks_ago: int):
...
def get_raw_shift_instance_data() -> str:
    ...
    """
    raw_data = (
        ShiftInstance.query.filter(ShiftInstance.due_date <= datetime.today())
        .join(ShiftInstance.shift)
    ...
    rows = [
        (
            [
                ...
                shift_instance.completed_timestamp.strftime("%T")
                if shift_instance.completed_timestamp is not None
                else "null",
                str(shift_instance.eggs_taken_home) if shift_instance.eggs_taken_home is not None else "null",
                shift_instance.edited_timestamp.strftime("%T")
                if shift_instance.edited_timestamp is not None
                else "null",
            ]
        )
...
def test_filter():
    ...
    data = ShiftInstance.query.filter(
        and_(
            ShiftInstance.due_date >= cutoff_date,
            ShiftInstance.eggs_taken_home.isnot(None),
        )
...
def get_table_data() -> dict[str, list[Shift]]:
    ...
    morning_shifts = (
        Shift.query.filter(Shift.time_of_day == TimeOfDayEnum.MORNING)
        .order_by(
            Shift.day_of_week,
    ...
    evening_shifts = (
        Shift.query.filter(Shift.time_of_day == TimeOfDayEnum.EVENING)
        .order_by(
            Shift.day_of_week,
            Shift.time_of_day.desc(),
        )
...
def set_sunrise_sunset_on_all():
    shift_instances = (
        ShiftInstance.query.filter(and_(ShiftInstance.sunrise_utc.is_(None), ShiftInstance.sunset_utc.is_(None)))
        .join(ShiftInstance.shift)
        .all()
    )
